...
import {
    Plus, X, MapPin, MessageCircle, HeartHandshake,
...
const Petition = () => {
    ...
    const fetchPosts = useCallback(async () => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                // Filter only petitions, exclude adopted ones
                fetchedPosts = fetchedPosts.filter(p => p.isPetition && !p.status.includes('adopted'));
    ...
    // Body scroll lock moved to ReportForm.js
    ...
    // ─── ADOPT PETITION (API) ───
    const handleAdoptPetition = async (postId) => {
        if (!window.confirm("Are you sure your organization wants to adopt this problem? This commits you to resolving it.")) return;
        
        try {
            const res = await fetch(`${API_BASE}/posts/${postId}/adopt/`, {
                method: 'POST',
                credentials: 'include',
                headers: { 'Content-Type': 'application/json' }
            });
            ...
            if (res.ok) {
                setPosts(prev => prev.map(post =>
                    post.id === postId ? {
                        ...post,
                        status: 'adopted and pending',
                        adoptedBy: data.post.adoptedBy,
                        adoptedDate: data.post.adoptedDate
                    } : post
                ));
            } else {
                    ...
                    <div style={{ marginBottom: '1rem', padding: '0.8rem', background: 'rgba(0,0,0,0.02)', border: `1px solid ${T.border}`, position: 'relative' }}>
                        ...
                        <button onClick={() => handleSignPetition(post.id)} disabled={post.hasSigned}
                            style={{
                                ...
                                marginBottom: (currentUserRole === 'ngo' || currentUserRole === 'nss') && post.status === 'pending' ? '0.5rem' : 0
                            }}>
                            <AlertTriangle size={14} />
                            {post.hasSigned ? 'You Signed This' : 'SIGN PETITION'}
                        </button>
                        
                        {(currentUserRole === 'ngo' || currentUserRole === 'nss') && post.status === 'pending' && (
                            <button onClick={() => handleAdoptPetition(post.id)}
                                style={{
                                    width: '100%', padding: '0.5rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                                    fontSize: '0.75rem', fontWeight: 'bold', fontFamily: T.fontSpecial, letterSpacing: '1px',
                ...
                <div style={{ display: 'flex', gap: '1.5rem', flexWrap: 'wrap' }}>
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <div style={{ marginBottom: '1rem', padding: '1rem 1.2rem', background: T.paper, border: `1px solid ${T.border}`, boxShadow: `4px 4px 0 rgba(0,0,0,0.08)`, borderBottom: `3px solid ${T.gold}` }}>
                            <h1 style={{ fontFamily: T.fontSpecial, fontSize: '1.5rem', color: T.ink, letterSpacing: '2px', textTransform: 'uppercase', margin: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                                <Flame size={22} color={T.red} />
                                Active <em style={{ color: T.red, fontStyle: 'normal' }}>Petitions</em>
                            </h1>
                            <p style={{ fontSize: '0.8rem', color: T.border, fontFamily: T.fontMono, marginTop: '0.3rem', textAlign: 'center' }}>
                                Critical issues that need immediate community support and signatures
                            </p>
                        </div>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            {loading ? (
                                ...
                                <div style={{ background: T.paper, border: `1px solid ${T.border}`, padding: '3rem', textAlign: 'center', boxShadow: `4px 4px 0 ${T.coffeeDk}` }}>
                                    ...
                                    <h3 style={{ fontFamily: T.fontSpecial, fontSize: '1.1rem', color: T.ink, marginBottom: '0.4rem' }}>No petitions yet</h3>
